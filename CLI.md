...
1-4. 設定を保存します。このときアクチュエーションポイントだけでなくキーマップなどすべてが同時にセーブされます。なので、1-2 でベースとするキーマップをロードしておく必要があります。  
...
* MOKA には磁気スイッチをコントロールするサブマイコンが 6つ搭載されています。  
それぞれ赤枠のキーを担当していて、左側から 0～5 の番号が振られています。  
...
今現在の磁気センサー読み取り値の生数値
* rawrelease
磁気スイッチのリリース状態時のセンサー値（MOKA起動時に測定）
* rawpress
磁気スイッチの押し込み状態時のセンサー値（キャリブレーションで測定）
磁気スイッチが押されている位置を 0～255 に変換した値
